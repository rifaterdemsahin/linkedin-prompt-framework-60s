# 🎬 LinkedIn 60s — Prompt Framework (🎭 Role · 📖 Context · 🚧 Constraints · ✅ Task)

📦 Production pack for a 60-second LinkedIn talking-head: 🟩 green-screen A-roll, 📜 teleprompter, 🗺️ preproduction plan, 🧪 sanity check, and a 🎥 shoot → ✂️ edit → 🚀 upload checklist.

## 🌐 GitHub Pages

🔗 **https://rifaterdemsahin.github.io/linkedin-prompt-framework-60s/**

📂 Repo: https://github.com/rifaterdemsahin/linkedin-prompt-framework-60s

## 🎥 Use on set

**📺 Elgato Prompter (Camera Hub Text mode)**

1. 🌐 Open the live page and click **📋 Copy for Elgato Prompter**.
2. 🖥️ Camera Hub → Prompter → Text → **+** → name it `Prompt framework 60s`.
3. 📋 Paste. Blank lines become chapters (🎣 Hook, 🔄 Shift, 🧱 Framework, 🎭 Role, 📖 Context, 🚧 Constraints, ✅ Task, 📣 CTA).
4. 🔠 Font large, 🐢 scroll slow, 🎯 lens overlay on.

🖥️ Web prompter is the backup: Open Prompter on the page. ⏯️ Space = play/pause. 🪞 `M` = mirror. ⚡ `+` / `-` = speed.

🏫 Skool CTA: [Delivery Pilot](https://www.skool.com/delivery-pilot-8938/)
